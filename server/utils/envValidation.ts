...
export function validateEnvironment(): ValidationResult {
  ...
  // Check AI service keys (optional - app will start but AI features won't work)
  const aiKeys = [
    'OPENAI_API_KEY',
    'ANTHROPIC_API_KEY', 
    'DEEPSEEK_API_KEY',
  ...
  if (configuredAIKeys.length === 0) {
    warnings.push("No AI service API keys configured - AI features will not work until you add: " + aiKeys.join(", "));
    optionalMissing += aiKeys.length;
  } else {
